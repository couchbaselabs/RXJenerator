...
public class RXJenerator 
{
	...
	public RXJenerator() {
		...
		if (retryStrategy == RetryStrategy.BEST_EFFORT) env = DefaultCouchbaseEnvironment.builder().retryStrategy(BestEffortRetryStrategy.INSTANCE).build();
		else if(retryStrategy == RetryStrategy.FAST_FAIL) env = DefaultCouchbaseEnvironment.builder().retryStrategy(FailFastRetryStrategy.INSTANCE).build();
	...
	public void bulkWrite() {
		//Observable.interval(sleep_time, TimeUnit.MILLISECONDS, Schedulers.newThread()) // this opens a memory leak
		Observable.interval(sleep_time, TimeUnit.MILLISECONDS, Schedulers.io()) //This is ok AND very smooth :-)
		.map(tick -> { 
				return Observable.from(docs)
					...
					.flatMap(doc -> { 						//returns a new observable for each emission - so that exceptions can be caught per write - allows for continuation of upstream observable
						final long s = System.nanoTime(); 	//value is reset per document
						return bucket.async().upsert(doc) 	//.upsert(doc, PersistTo.MASTER, ReplicateTo.ONE) //Waiting for bug fix
		...
		System.out.println("Bulk Write returns immediately");
	...
	public void bulkGet() {
		...
		.map(tick -> {
		    ...
		    .observeOn(Schedulers.newThread()) //this will create a new thread for the behavior below, i.e. just the logging) - CONFIRM
		    .subscribe(	    		
	    		returnedDocument -> {
	...
	public Observable<JsonDocument> getWithFallback(String id) {
	    ...
	    .onErrorResumeNext((error) -> {
		    	return bucket.async().getFromReplica(id, ReplicaMode.FIRST).first() // This works with BestEffortRetryStrategy AND FailFastRetryStrategy (don't need .first()
